    ...
    extractTextFromPDF(url).then(fullText => {
        ...
        if (dateMatch) {
            ...
            if (dateElement) {
                ...
                console.error("L'élément de date n'a pas été trouvé dans le formulaire.");
            }
        } else {
            console.error("Aucune date de document trouvée.");
        }

        // Match la date de naissance explicitement à partir du regex
        let dateOfBirthMatch = fullText.match(dateOfBirthRegex);
        if (dateOfBirthMatch) {
            dateOfBirth = moment(dateOfBirthMatch[1], "DD/MM/YYYY"); // Extraction de la date de naissance
            console.log("Found date of birth: " + dateOfBirth.format("DD/MM/YYYY"));
        } else {
            console.warn("Aucune date de naissance trouvée.");
        }

        // Match les noms extraits du document
        var nameMatches = [];
        let nameMatchesIterator = fullText.matchAll(firstNameRegex);
        Array.from(nameMatchesIterator).forEach(match => nameMatches.push(match[1]));

        // Si aucun nom n'est trouvé, essayer le regex de secours
        ...
        if (nameMatches.length > 0) {
            console.log("Nom(s) trouvé(s): " + nameMatches);
        } else {
            console.warn("Aucun nom trouvé.");
        }

        // Appeler confirmAndFill avec les données extraites si elles sont valides
        if (documentDate && nameMatches.length > 0) {
            confirmAndFill(documentDate, dateOfBirth, nameMatches);
        } else {
            console.error("Les données requises n'ont pas été trouvées. Confirmation ignorée.");
        }
    }).catch(error => {
