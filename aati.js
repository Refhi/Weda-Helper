...
addTweak('https://secure.weda.fr/FolderMedical/Aati.aspx', 'autoAATI', function () {
    ...
    function clickATWithOrWithoutCV(elements) {
    ...
    lightObserver(selecteurBoutonCV, clickATWithOrWithoutCV, document, true);
...
addTweak('https://secure.weda.fr/BinaryData.aspx', "*sendDocToCompanion", function () {
    getOption(['RemoveLocalCompanionPrint', 'autoAATIexit'], function ([RemoveLocalCompanionPrint, autoAATIexit]) {
        let demandeImpressionATrecente = Date.now() - autoAATIexit < 10000;
        if (demandeImpressionATrecente && RemoveLocalCompanionPrint === false) {
            console.log('autoAATIexit', autoAATIexit, 'is less than 10 seconds ago');
        ...
        } else if (demandeImpressionATrecente){
